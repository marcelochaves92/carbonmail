from carbonmail.database.operations import select
import csv
...
def create_list(list_name):
    if not string_null_or_empty(list_name):
        return False
...
def import_contacts(csv_path,list_name):
    ...
    with open (csv_path,'r',encoding='UTF-8') as csv_file:
        dialect = csv.Sniffer().sniff(csv.read(1024))
        csv_file.seek(0)
